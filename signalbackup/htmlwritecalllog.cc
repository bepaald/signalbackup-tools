  ...
  */
  ...
  for (unsigned int i = 0; i < results.rows(); ++i)
  {
    ...
    if (peer > -1 && !bepaald::contains(peers, peer))
    {
      if (getRecipientInfoFromMap(recipientinfo, peer).hasavatar)
      {
        ...
        std::string avatar_path(sanitizeFilename(raw_avatar_path, d_aggressive_filename_sanitizing) + " (_id" + bepaald::toString(threadid) + ")");
        if (compact) [[unlikely]]
          avatar_path = "id" + bepaald::toString(threadid);
  ...
  for (unsigned int i = 0; i < results.rows(); ++i)
  {
    ...
    */
    ...
    outputfile <<
      "<span class=\"middot\">&middot;</span>" << date_date << "</div>\n"
      "            </div>\n"
      "          </div>\n";
