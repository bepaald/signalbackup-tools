...
std::string SignalBackup::decodeStatusMessage(std::string const &body, long long int expiration, long long int type, std::string const &contactname, IconType *icon) const
{
  ...
  if (Types::isGroupUpdate(type) && !Types::isGroupV2(type))
  {
    ...
    std::string members;
    auto const &field4 = statusmsg.getFieldView<4>();
    ...
    std::string_view title = statusmsg.getFieldView<3>().value_or(std::string_view());
    if (!title.empty())
    {
      result += (!members.empty() ? ' ' : '\n');
      result.append("Group name is now '").append(title).append("'.");
    }
    return result;
  ...
  if (Types::isIdentityUpdate(type))
  {
    ...
    return bepaald::concat("Your safety number with ", contactname, " has changed.");
  }
  ...
  if (Types::isProfileChange(type))
  {
    ...
    return decodeProfileChangeMessage(profchangefull/*body*/, contactname, icon);
  }
  ...
  if (Types::isThreadMergeType(type))
  {
    ...
    */
    ...
    auto field1 = thread_merge_event.getFieldView<1>();
    if (field1.has_value())
      previous_e164.append(field1.value());

    return bepaald::concat("Your message history with ", contactname, " and their number", previous_e164, " has been merged.");
  }
  ...
  if (Types::isGroupUpdate(type) && Types::isGroupV2(type)) // see app/src/test/java/org/thoughtcrime/securesms/database/model/GroupsV2UpdateMessageProducerTest.java
  {
    ...
    return decodeGroupV2UpdateMessage(groupv2ctx, type, contactname, icon);
  ...
  //   std::cout << "GV1_MIGRATION_TYPE!!!" << std::endl;
  //   std::cout << Types::isGroupUpdate(type)  << std::endl;
  //   std::cout << Types::isGroupV2(type) << std::endl;
...
std::string SignalBackup::decodeStatusMessage(std::pair<std::shared_ptr<unsigned char []>, size_t> const &body,
                                              long long int type, std::string const &contactname, IconType *icon) const
{
  ...
  */
  ...
  auto field1 = me.getFieldView<1>();
  if (field1.has_value()) // GV2UpdateDescription
  {
    auto field1_1 = field1->getFieldView<1>(); // DecryptedGroupV2Context
    if (field1_1.has_value())
      return decodeGroupV2UpdateMessage(field1_1.value()/*->getDataString()*/, type, contactname, icon);
  }

  auto field3 = me.getFieldView<3>(); // ProfileChangeDetails
  if (field3.has_value())
    return decodeProfileChangeMessage(field3.value()/*->getDataString()*/, contactname, icon);

  auto field5 = me.getFieldView<5>(); // PollTerminate
  if (field5.has_value())
    return decodePollTerminateMessage(field5.value(), type, contactname, icon);

  auto field6 = me.getFieldView<6>(); // PinnedMessage
  if (field6.has_value())
  {
    if (icon && *icon == IconType::NONE)
