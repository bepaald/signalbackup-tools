                                         ...
                                         std::string const &exportdetails, bool compact) const
{
  ...
  if (bepaald::fileOrDirExists(dir + "/fullcontactslist.html"))
  {
    if (!overwrite && !append)
    {
      Logger::error("'", dir, "/fullcontactslist.html' exists. Use --overwrite to overwrite.");
      return false;
    }
  }
  ...
  outputfile <<
    ...
    "  <head>\n"
    "    <meta charset=\"utf-8\">\n"
    "    <title>Signal known contacts list</title>\n"
    "    <style>\n"
    ...
    "        --conversationlistitem-bc: " << (light ? "#E7EBF3;" : "#303133;") << "\n"
  ...
  for (unsigned int i = 0; i < results.rows(); ++i)
  {
    ...
    if (getRecipientInfoFromMap(recipient_info, rec_id).hasavatar)
    {
      ...
      {
        if (!bepaald::fileOrDirExists(bepaald::concat(dir, "/", "/media/Avatar_", bepaald::toString(rec_id), ".", avatar_extension))) // maybe htmlroot/media/avatar was already
  ...
  for (unsigned int ii = 0; ii < order.size(); ++ii)
  {
    ...
    outputfile <<
      ...
      "            <div class=\"name\"><pre>" << HTMLescapeString(getRecipientInfoFromMap(recipient_info, rec_id).display_name) << "</pre></div>\n";

    if (blocked != 0)
      outputfile << "            <div class=\"extrainfo\">Blocked</div>\n";

    if (hidden != 0)
      outputfile << "            <div class=\"extrainfo\">Hidden</div>\n";
    ...
    if (!isgroup) // groups do not have registration status (always 'unknown')
    {
      switch (registered)
      {
