...
// bool SignalBackup::migrate_to_191_CUSTOM(std::string const &selfphone)
// {
...
//   std::cout << "Doing a select:" << std::endl;
//   d_database.prettyPrint(d_truncate, "SELECT _id, date_received FROM message LIMIT 2");
//   std::cout << "Integrity check:" << std::endl;
//   d_database.prettyPrint(d_truncate, "SELECT * FROM pragma_integrity_check");
//   std::cout << "Attempting a delete on the message table:" << std::endl;
//   if (!d_database.exec("DELETE FROM message WHERE _id IS (SELECT min(_id) FROM message)"))
//     return false;
//   std::cout << (d_database.changed() ? "OK!" : "(?)") << std::endl;

//   d_database.saveToFile("v174");

//   if (d_databaseversion < 175)
//   {
//     Logger::message("To 175");

//     //d_database.printLineMode("SELECT name,sql FROM sqlite_schema WHERE tbl_name = 'message' AND type != 'table' ORDER BY name,sql");
...
//       //std::cout << "DROPPING: '" << "DROP " + res(i, "type") + " IF EXISTS " + res(i, "name") << "'" << std::endl;
...
//       {
//         //std::cout << "CREATE INDEX message_thread_story_parent_story_scheduled_date_latest_revision_id_index ON message (thread_id, date_received, story_type, parent_story_id, scheduled_date, latest_revision_id)' (hc)" << std::endl;
...
//         //std::cout << "CREATE INDEX message_quote_id_quote_author_scheduled_date_latest_revision_id_index ON message (quote_id, quote_author, scheduled_date, latest_revision_id)' (hc)" << std::endl;
...
//         //std::cout << "CREATE INDEX message_date_sent_from_to_thread_index ON message (date_sent, from_recipient_id, to_recipient_id, thread_id)' (hc)" << std::endl;
...
//           //std::cout << statement << "' (auto)" << std::endl;
...
//   // d_database.exec("DROP TABLE drafts; "
//   //                 "DROP TABLE group_receipts; "
//   //                 "DROP TABLE groups; "
//   //                 "DROP TABLE payments; "
//   //                 "DROP TABLE push; "
//   //                 "DROP TABLE remapped_threads; "
//   //                 "DROP TABLE remapped_recipients; "
//   //                 "DROP TABLE sessions; "
//   //                 "DROP TABLE signed_prekeys; "
//   //                 "DROP TABLE sms; "
//   //                 "DROP TABLE sms_fts; "
//   //                 "DROP TABLE sms_fts_config; "
//   //                 "DROP TABLE sms_fts_data; "
//   //                 "DROP TABLE sms_fts_docsize; "
//   //                 "DROP TABLE sms_fts_idx; "
//   //                 "DROP TABLE sticker; "
//   //                 "DROP TABLE part; "
//   //                 "DROP TABLE storage_key; ");

//   // d_database.print("SELECT COUNT(*) FROM message");

//   // d_database.print("SELECT * FROM pragma_integrity_check");
//   // if (!d_database.exec("DELETE FROM message WHERE _id IS (SELECT min(_id) FROM message)"))
//   //   return false;
//   // std::cout << d_database.changed() << std::endl;
...
//     // d_database.prettyPrint(d_truncate, "SELECT _id FROM message M WHERE "
//     //                        "  _id > "
//     //                        "  ("
